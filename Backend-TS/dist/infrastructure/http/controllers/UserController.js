...
const EmailGeneratorService_1 = require("../../../domain/services/EmailGeneratorService");
const UserValidator_1 = require("../../../domain/value-objects/UserValidator");
const postgres_1 = require("../../database/postgres");
const bcrypt_1 = __importDefault(require("bcrypt"));
const crypto_1 = __importDefault(require("crypto"));
const userRepository = new UserRepository_1.UserRepository();
const emailGeneratorService = new EmailGeneratorService_1.EmailGeneratorService(userRepository);
const createUserUseCase = new CreateUserUseCase_1.CreateUserUseCase(userRepository, emailGeneratorService);
...
class UserController {
    static async create(req, res) {
        try {
            ...
            if (!targetPersonaId) {
                ...
                UserValidator_1.UserValidator.validateIdentificacion(identificacion);
    ...
    static async getAll(req, res) {
        try {
            ...
            const normalizedUsers = users.map((user) => sanitizeUser(user));
    ...
    static async update(req, res) {
        try {
            ...
            const updateData = {
                ...
                password: req.body.password ? await bcrypt_1.default.hash(req.body.password, 10) : undefined
            };
    ...
    static async importBulk(req, res) {
        try {
            ...
            for (const rawRow of rows) {
                ...
                const temporaryPassword = rawRow.password || crypto_1.default.randomBytes(6).toString('hex');
    ...
    static async getDashboardStats(req, res) {
        try {
            const activosResult = await postgres_1.pool.query("SELECT COUNT(*) FROM usuarios WHERE SessionActive = '1'");
            const inactivosResult = await postgres_1.pool.query("SELECT COUNT(*) FROM usuarios WHERE SessionActive = '0'");
            const bloqueadosResult = await postgres_1.pool.query("SELECT COUNT(*) FROM usuarios WHERE Status = 'BLOQUEADO'");
            const fallidosResult = await postgres_1.pool.query("SELECT SUM(intentos_fallidos) as total_fallidos FROM usuarios");
            return res.status(200).json({
