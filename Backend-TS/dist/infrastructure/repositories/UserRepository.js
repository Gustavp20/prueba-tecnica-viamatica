...
class UserRepository {
    async findByEmail(email) {
        ...
        const result = await postgres_1.pool.query(query, [email]);
        return result.rows[0];
    }
    async findByUsername(username) {
        const query = 'SELECT * FROM usuarios WHERE UserName = $1 AND deleted_at IS NULL';
        const result = await postgres_1.pool.query(query, [username]);
        return result.rows[0];
    }
    ...
    async getRoleIdByName(roleName) {
        ...
        const result = await postgres_1.pool.query(query, [roleName]);
        return result.rows[0]?.idrol || null;
    }
    async assignRoleToUser(userId, roleName = 'USER') {
        ...
        const query = `
            ...
            RETURNING *;
        `;
        await postgres_1.pool.query(query, [roleId, userId]);
    }
    async syncUserRole(userId, roleName) {
        const roleId = await this.getRoleIdByName(roleName);
        if (!roleId) {
            throw new Error(`No existe el rol ${roleName}. Ejecuta primero la semilla de roles.`);
        }
        ...
        try {
            ...
            await client.query('INSERT INTO rol_usuarios (Rol_idRol, usuarios_idUsuario) VALUES ($1, $2)', [roleId, userId]);
    ...
    async findLastFailedAttemptsByUserId(userId) {
        const query = `
    ...
    async findPersonaByIdentification(identificacion) {
        const query = 'SELECT * FROM Persona WHERE Identificacion = $1 AND deleted_at IS NULL';
        const result = await postgres_1.pool.query(query, [identificacion]);
        return result.rows[0];
    }
    ...
    async findDetailedByIdentificationOrUserName(identificacion, userName, mail) {
        const query = `
            SELECT
                u.idUsuario AS id,
                u.UserName AS "userName",
            ...
            LIMIT 1
        `;
        const result = await postgres_1.pool.query(query, [identificacion, userName, mail]);
        return result.rows[0];
    }
    ...
    async update(id, updateData) {
        ...
        try {
            await client.query('BEGIN');
            const userResult = await client.query('SELECT Persona_idPersona2 FROM usuarios WHERE idUsuario = $1 AND deleted_at IS NULL FOR UPDATE', [id]);
            if (!userResult.rows[0]) {
                throw new Error('Usuario no encontrado.');
            }
            const personaId = userResult.rows[0].persona_idpersona2;
            const personaFieldsTouched = updateData.nombres !== undefined ||
            ...
            const userFieldsTouched = updateData.userName !== undefined ||
    ...
    async findByPersonaId(personaId) {
        ...
        const result = await postgres_1.pool.query(query, [personaId]);
        return result.rows[0];
    }
