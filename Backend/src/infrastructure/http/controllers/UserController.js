...
const EmailGeneratorService = require('../../../domain/services/EmailGeneratorService');
...
class UserController {
    ...
    static async getMe(req, res) {
        try {
            ...
            res.status(500).json({ message: 'Error al obtener tu información.' });
        }
    }
    ...
    static async getMenu(req, res) {
        try {
            ...
            res.status(200).json(normalizedMenu);
        } catch (error) {
    ...
    static async updateRole(req, res) {
        try {
            ...
            const targetUser = await userRepository.findDetailedById(id);
            if (!targetUser) {
                return res.status(404).json({ message: 'Usuario no encontrado.' });
            }
            ...
            await userRepository.syncUserRole(id, roleName);

            const updatedUser = await userRepository.findDetailedById(id);
    ...
    static async update(req, res) {
        try {
            ...
            res.status(200).json({ message: "Usuario actualizado", user: sanitizeUser(updatedUser) });
        } catch (error) {
    ...
    static async importBulk(req, res) {
        try {
            ...
            res.status(200).json({ message: 'Carga masiva procesada correctamente.', ...result });
        } catch (error) {
            res.status(500).json({ message: error.message || 'Error al procesar la carga masiva.' });
        }
    }
    ...
    static async getDashboardStats(req, res) {
        try {
            // Consultas rápidas para los indicadores del Dashboard
            ...
            res.status(500).json({ message: "Error al cargar estadísticas del dashboard." });
        }
    }
}
